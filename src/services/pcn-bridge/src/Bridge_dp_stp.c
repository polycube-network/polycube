...
u32 stp_forward_in_state(u32 state)
{
 ...
 * port should still work, just not have STP applied to it. */
static __always_inline
u32 stp_learn_in_state(u32 state)
{
...
BPF_TABLE("hash", struct port_vlan_key, struct port_vlan_value, port_vlan, 1024);
/*
...
/*
 * Fowarding Table: VLAN + MAC Address to port association.  This table is filled
...
int handle_rx(struct CTXTYPE *ctx, struct pkt_metadata *md) {
  ...
  pcn_log(ctx, LOG_DEBUG, "in_ifc=%d proto=0x%x mac src:%M dst:%M", in_ifc, bpf_ntohs(eth->proto), eth->src, eth->dst);
  ...
  if (in_port->mode == PORT_MODE_ACCESS) {
      ...
      pcn_log(ctx, LOG_TRACE, "port mode ACCESS vlan=%d, state=%d", vlanid, vlan_entry->stp_state);
  } else {
    // TRUNK
    if (!tagged) {
  ...
  if (!stp_learn_in_state(vlan_entry->stp_state)) {
    pcn_log(ctx, LOG_TRACE, "port %d should not learn: state: %d", in_ifc, vlan_entry->stp_state);
    return RX_DROP;
  }
