...
OVS_PACKED(struct llc_header {
  uint8_t llc_dsap;
  uint8_t llc_ssap;
  uint8_t llc_cntl;
});

static const uint8_t eth_addr_stp[ETH_ADDR_LEN] = {0x01, 0x80, 0xC2,
                                                   0x00, 0x00, 0x00};

// TODO: is it ok to use a different mutex for each instance?
static pthread_mutex_t mutex;

OVS_PACKED(struct stp_bpdu_header {
...
OVS_PACKED(struct stp_config_bpdu {
...
OVS_PACKED(struct stp_tcn_bpdu {
...
static struct stp_port *stp_next_enabled_port(const struct stp *stp,
                                              const struct stp_port *port)
/*OVS_REQUIRES(mutex)*/
{
...
static bool stp_supersedes_root(
    const struct stp_port *root,
...
struct stp *stp_ref(const struct stp *stp_) {
...
void stp_tick(struct stp *stp, int ms) {
  struct stp_port *p;
  int elapsed;
...
static void set_bridge_id(struct stp *stp, stp_identifier new_bridge_id)
/*OVS_REQUIRES(mutex)*/
{
...
void stp_set_bridge_id(struct stp *stp, stp_identifier bridge_id) {
...
void stp_set_bridge_priority(struct stp *stp, uint16_t new_priority) {
  const uint64_t mac_bits = (UINT64_C(1) << 48) - 1;
  ovs_mutex_lock(&mutex);
  set_bridge_id(stp,
                ((stp->bridge_id & mac_bits) | ((uint64_t)new_priority << 48)));
  ovs_mutex_unlock(&mutex);
}
...
void stp_set_hello_time(struct stp *stp, int ms) {
  ovs_mutex_lock(&mutex);
...
void stp_set_max_age(struct stp *stp, int ms) {
  ovs_mutex_lock(&mutex);
  stp->rq_max_age = ms;
  stp_update_bridge_timers(stp);
  ovs_mutex_unlock(&mutex);
...
void stp_set_forward_delay(struct stp *stp, int ms) {
  ovs_mutex_lock(&mutex);
...
const char *stp_get_name(const struct stp *stp) {
  char *name;
...
stp_identifier stp_get_bridge_id(const struct stp *stp) {
...
stp_identifier stp_get_designated_root(const struct stp *stp) {
 ...
 * false otherwise. */
bool stp_is_root_bridge(const struct stp *stp) {
  bool is_root;
...
int stp_get_root_path_cost(const struct stp *stp) {
  int cost;
...
int stp_get_hello_time(const struct stp *stp) {
  int time;
...
int stp_get_max_age(const struct stp *stp) {
  int time;
...
int stp_get_forward_delay(const struct stp *stp) {
  int time;
...
bool stp_check_and_reset_fdb_flush(struct stp *stp) {
  bool needs_flush;
 ...
 * STP_MAX_PORTS. */
struct stp_port *stp_get_port(struct stp *stp, int port_no) {
...
struct stp_port *stp_get_root_port(struct stp *stp) {
  struct stp_port *port;
...
bool stp_get_changed_port(struct stp *stp, struct stp_port **portp) {
...
bool stp_should_forward_bpdu(enum stp_state state) {
  return (state &
          (STP_DISABLED | STP_LISTENING | STP_LEARNING | STP_FORWARDING)) != 0;
}

/* Returns the name for the given 'role' (for use in debugging and log
 * messages). */
const char *stp_role_name(enum stp_role role) {
  switch (role) {
  case STP_ROLE_ROOT:
    return "root";
...
void stp_received_bpdu(struct stp_port *p, const void *bpdu, size_t bpdu_size) {
  ...
  if (bpdu_size < sizeof(struct stp_bpdu_header)) {
    VLOG_WARN("[%s]: received runt %" PRIuSIZE "-byte BPDU", stp->name,
              bpdu_size);
    p->error_count++;
    goto out;
  }
  ...
  default:
    VLOG_WARN("[%s]: received BPDU of unexpected type %" PRIu8, stp->name,
              header->bpdu_type);
    p->error_count++;
    goto out;
  }
...
struct stp *stp_port_get_stp(struct stp_port *p) {
  struct stp *stp;
...
void stp_port_set_name(struct stp_port *p, const char *name) {
  char *old;
 ...
 * state. */
void stp_port_set_aux(struct stp_port *p, void *aux) {
  ovs_mutex_lock(&mutex);
  p->aux = aux;
  ovs_mutex_unlock(&mutex);
}
...
void *stp_port_get_aux(struct stp_port *p) {
  void *aux;
...
int stp_port_no(const struct stp_port *p) {
  struct stp *stp;
  int index;
...
int stp_port_get_id(const struct stp_port *p) {
  int port_id;
...
enum stp_state stp_port_get_state(const struct stp_port *p) {
...
enum stp_role stp_port_get_role(const struct stp_port *p) {
...
void stp_port_get_counts(const struct stp_port *p, int *tx_count, int *rx_count,
                         int *error_count) {
  ovs_mutex_lock(&mutex);
  *tx_count = p->tx_count;
  *rx_count = p->rx_count;
...
void stp_port_disable(struct stp_port *p) {
  struct stp *stp;
...
void stp_port_enable(struct stp_port *p) {
  ovs_mutex_lock(&mutex);
  if (p->state == STP_DISABLED) {
...
void stp_port_set_priority(struct stp_port *p, uint8_t new_priority) {
...
void stp_port_set_path_cost(struct stp_port *p, uint16_t path_cost) {
  ovs_mutex_lock(&mutex);
...
void stp_port_set_speed(struct stp_port *p, unsigned int speed) {
  stp_port_set_path_cost(p, stp_convert_speed_to_cost(speed));
}

/* Enables topology change detection on port 'p'. */
void stp_port_enable_change_detection(struct stp_port *p) {
  p->change_detection_enabled = true;
}

/* Disables topology change detection on port 'p'. */
void stp_port_disable_change_detection(struct stp_port *p) {
  p->change_detection_enabled = false;
}

static void stp_transmit_config(struct stp_port *p) /*OVS_REQUIRES(mutex)*/
{
  struct stp *stp = p->stp;
  bool root = stp_is_root_bridge(stp);
  ...
  if (p->hold_timer.active) {
    VLOG_DBG("[%s], port: %s, transmit config bpdu pending", stp->name,
             p->port_name);
    ...
    if (ntohs(config.message_age) < stp->max_age) {
      p->topology_change_ack = false;
      p->config_pending = false;
      VLOG_DBG("[%s], port: %s, transmit config bpdu", stp->name, p->port_name);
...
static bool stp_supersedes_port_info(const struct stp_port *p,
...
static void stp_record_config_information(struct stp_port *p,
                                          const struct stp_config_bpdu *config)
/*OVS_REQUIRES(mutex)*/
{
...
static void stp_record_config_timeout_values(
    struct stp *stp, const struct stp_config_bpdu *config)
/*OVS_REQUIRES(mutex)*/
{
...
static bool stp_is_designated_port(
    const struct stp_port *p) /*OVS_REQUIRES(mutex)*/
{
  return (p->designated_bridge == p->stp->bridge_id &&
          p->designated_port == p->port_id);
}

static void stp_config_bpdu_generation(struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
  struct stp_port *p;
...
static void stp_transmit_tcn(struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
  ...
  VLOG_DBG("[%s], root port: %s, transmit tcn", stp->name, p->port_name);
...
static void stp_configuration_update(struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
...
static void stp_root_selection(struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
...
static void stp_designated_port_selection(
    struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
  struct stp_port *p;
...
static void stp_become_designated_port(
    struct stp_port *p) /*OVS_REQUIRES(mutex)*/
{
  struct stp *stp = p->stp;
  p->designated_root = stp->designated_root;
...
static void stp_port_state_selection(struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
  struct stp_port *p;
...
static void stp_make_forwarding(struct stp_port *p) /*OVS_REQUIRES(mutex)*/
{
...
static void stp_make_blocking(struct stp_port *p) /*OVS_REQUIRES(mutex)*/
{
...
static void stp_set_port_state(struct stp_port *p, enum stp_state state)
/*OVS_REQUIRES(mutex)*/
{
...
static void stp_topology_change_detection(
    struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
  ...
  // seq_change(connectivity_seq_get());
  VLOG_DBG("[%s]: detected topology change.", stp->name);
}

static void stp_topology_change_acknowledged(
    struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
  stp->topology_change_detected = false;
  stp_stop_timer(&stp->tcn_timer);
...
static void stp_acknowledge_topology_change(
    struct stp_port *p) /*OVS_REQUIRES(mutex)*/
{
...
static void stp_received_config_bpdu(struct stp *stp, struct stp_port *p,
                                     const struct stp_config_bpdu *config)
/*OVS_REQUIRES(mutex)*/
{
...
static void stp_received_tcn_bpdu(struct stp *stp, struct stp_port *p)
/*OVS_REQUIRES(mutex)*/
{
  if (p->state != STP_DISABLED) {
    if (stp_is_designated_port(p)) {
...
static void stp_hello_timer_expiry(struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
  stp_config_bpdu_generation(stp);
  stp_start_timer(&stp->hello_timer, 0);
}

static void stp_message_age_timer_expiry(
    struct stp_port *p) /*OVS_REQUIRES(mutex)*/
{
  struct stp *stp = p->stp;
  bool root = stp_is_root_bridge(stp);

  VLOG_DBG("[%s], port: %s, message age timer expired", stp->name,
           p->port_name);
  stp_become_designated_port(p);
  stp_configuration_update(stp);
  stp_port_state_selection(stp);
...
static bool stp_is_designated_for_some_port(
    const struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
  const struct stp_port *p;
...
static void stp_forward_delay_timer_expiry(
    struct stp_port *p) /*OVS_REQUIRES(mutex)*/
{
...
static void stp_tcn_timer_expiry(struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
  stp_transmit_tcn(stp);
  stp_start_timer(&stp->tcn_timer, 0);
}

static void stp_topology_change_timer_expiry(
    struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
  stp->topology_change_detected = false;
  stp->topology_change = false;
...
static void stp_hold_timer_expiry(struct stp_port *p) /*OVS_REQUIRES(mutex)*/
{
  if (p->config_pending) {
    stp_transmit_config(p);
  }
...
static void stp_initialize_port(struct stp_port *p, enum stp_state state)
/*OVS_REQUIRES(mutex)*/
{
...
static void stp_become_root_bridge(struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
  stp->max_age = stp->bridge_max_age;
  stp->hello_time = stp->bridge_hello_time;
  stp->forward_delay = stp->bridge_forward_delay;
...
static void stp_start_timer(struct stp_timer *timer,
                            int value) /*OVS_REQUIRES(mutex)*/
{
  timer->value = value;
  timer->active = true;
}

static void stp_stop_timer(struct stp_timer *timer) /*OVS_REQUIRES(mutex)*/
{
  timer->active = false;
}

static bool stp_timer_expired(struct stp_timer *timer, int elapsed, int timeout)
/*OVS_REQUIRES(mutex)*/
{
  if (timer->active) {
    timer->value += elapsed;
...
static void stp_update_bridge_timers(struct stp *stp) /*OVS_REQUIRES(mutex)*/
{
  int ht, ma, fd;
...
static void stp_send_bpdu(struct stp_port *p, const void *bpdu,
                          size_t bpdu_size)
/*OVS_REQUIRES(mutex)*/
{
  ...
  eth = ofpbuf_base(pkt);
  llc = ofpbuf_base(pkt) + sizeof(*eth);
  ...
  /* BPDU */
  void *l3 = (char *)ofpbuf_base(pkt) + ETH_HEADER_LEN + LLC_HEADER_LEN;
  memcpy(l3, bpdu, bpdu_size);
