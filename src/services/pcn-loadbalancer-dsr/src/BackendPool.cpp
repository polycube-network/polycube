...
BackendPool::BackendPool(Backend &parent, const BackendPoolJsonObject &conf): parent_(parent) {
...
void BackendPool::update(const BackendPoolJsonObject &conf) {
  //This method updates all the object/parameter in BackendPool object specified in the conf JsonObject.
...
void BackendPool::create(Backend &parent, const uint32_t &id, const BackendPoolJsonObject &conf){

  //This method creates the actual BackendPool object given thee key param.
  //Please remember to call here the create static method for all sub-objects of BackendPool.
  try {
    auto config_table = parent.parent_.get_array_table<uint64_t>("config_table");
    ...
    parent.logger()->error("[BackendPool] Error while creating the backend pool {0}", id);
    throw;
  }

}

std::shared_ptr<BackendPool> BackendPool::getEntry(Backend &parent, const uint32_t &id){
  //This method retrieves the pointer to BackendPool object specified by its keys.
...
void BackendPool::removeEntry(Backend &parent, const uint32_t &id){
  ...
  //Remember to call here the remove static method for all-sub-objects of BackendPool.
