...
Ports::Ports(polycube::service::Cube<Ports> &parent,
  ...
  logger()->debug("Creating a new port with type: {0} and name: {1} ",
    PortsJsonObject::PortsTypeEnum_to_string(conf.getType()), port->name());
...
void Ports::create(Lbdsr &parent, const std::string &name, const PortsJsonObject &conf){
...
void Ports::setType(const PortsTypeEnum &value){
  ...
  throw std::runtime_error("[Ports]: Type can be set only during port creation");
}
