...
Service::Service(K8switch &parent, const ServiceJsonObject &conf): parent_(parent),
backend_size_(INITIAL_BACKEND_SIZE), kube_proxy_service_(false) {
  vip_ = conf.getVip();
...
void Service::addBackend(const std::string &ip, const uint16_t &port,
  ...
  if (!kube_proxy_service_ && type_ == ServiceType::CLUSTER_IP) {
    ...
    for (auto &&service : parent_.service_map_) {
      ...
      if (service.second.service_backends_.count(key) != 0) {
        logger()->warn("Backend {}:{} is present in {}:{}/{}, defaulting to kube-proxy",
                      ip, port,
                      service.second.getVip(), service.second.getVport(),
                      ServiceJsonObject::ServiceProtoEnum_to_string(service.second.getProto()));
        removeServiceFromKernelMap();
  ...
  bool inserted;
  std::tie(iter, inserted) = service_backends_.emplace(std::piecewise_construct,
...
std::shared_ptr<ServiceBackend> Service::getBackend(const std::string &ip, const uint16_t &port) {
  ...
  return std::shared_ptr<ServiceBackend>(&service_backends_.at(key), [](ServiceBackend*){});
}
...
std::vector<std::shared_ptr<ServiceBackend>> Service::getBackendList() {
  ...
  for(auto &it : service_backends_) {
    auto ptr = std::shared_ptr<ServiceBackend>(&it.second, [](ServiceBackend*){});
...
void Service::removeServiceFromKernelMap() {
  ...
  if (type_ == ServiceType::CLUSTER_IP) {
    auto reverse_map = parent_.get_hash_table<backend, vip>(EBPF_BACKEND_TO_SERVICE_MAP);
...
std::vector<ServiceBackend::Key> Service::getConsistentArray() {
  ...
  *  if yes, we have to go the next row -> [1,0], retrieve the number in this cell,
  ...
  * 1) "-1" -> You can store the number, in this case I check if the entry can be
 ...
 * Then we multiply this coefficient for backend's weight in order to get the number of
...
std::map<ServiceBackend::Key, int> Service::getWeightBackend() {
  ...
  //logger()->trace("vec size {0} sum_weight {1} coeff {2} ", vect_size, sum_weight, coeff);
  ...
  for (auto &it : service_backends_) {
    ...
    //logger()->trace("backend {}:{} weight {}", key.first, key.second, weight_backend);
  }
  ...
  while (res > 0) {
    for (auto &it : service_backends_) {
      ...
      auto &key = it.first;
	    ...
	    //logger()->trace("backend {}:{} weight {}", key.first, key.second, weight_back_pool[key]);
...
void Service::insertAsKubeProxyService() {
  ...
  // This is a special value used to indicate that the service is implemented by kube-proxy
  backend value {
    .ip = utils::ip_string_to_be_uint(getVip()),
    .port = KUBE_PROXY_FLAG,
  };
