...
FwdTable::FwdTable(K8switch &parent, const FwdTableJsonObject &conf): parent_(parent) {
  logger()->info("Creating FwdTable instance");
}

FwdTable::FwdTable(K8switch &parent, const std::string &ip, const std::string &mac,
...
void FwdTable::create(K8switch &parent, const std::string &address, const FwdTableJsonObject &conf){
...
std::shared_ptr<FwdTable> FwdTable::getEntry(K8switch &parent, const std::string &address){
  uint32_t ip_key = get_index(address); // takes last part of the IP address
  ...
  try {
    ...
    parent.logger()->error("Unable to find FWD table entry for address {0}. {1}",
                            address, e.what());
...
std::vector<std::shared_ptr<FwdTable>> FwdTable::get(K8switch &parent){
  ...
  try{
    ...
    for(auto &entry: entries) {
      ...
      // TODO: key is only the index, it should be converted back to the full IP address
