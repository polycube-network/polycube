...
std::string cbpf2c::ToC(struct sock_fprog * cbpf) {
    /*
     * First I validate the cBPF code
     */
    if (cbpf_validate(cbpf) == 0)
        std::cout<<"Not a valid cBPF program!"<<std::endl;
    std::string ret = bpf_dump_all(cbpf);
    return ret;
}


std::string cbpf2c::bpf_dump_all(struct sock_fprog *bpf) {
    int i;
    std::string ret = "";
    for (i = 0; i < bpf->len; ++i) {
        /*
         ...
         */
        char *str = _cbpf_dump(bpf->filter[i], i);
        ret += std::string(str);
    }
    return ret;
}
...
char *cbpf2c::_cbpf_dump(const struct sock_filter bpf, int n)
{
    int v;
    const char *fmt, *op;
    static char ret[256];
          ...
          snprintf(ret, sizeof(ret), "L%d:\t%s %s) {\n\t\tgoto L%d;\n\t}else{\n\t\tgoto L%d;\n\t}\n", n, op, operand, n + 1 + bpf.jt, n + 1 + bpf.jf);
            ...
            if ((BPF_CLASS(bpf.code) == BPF_RET)) {
                if (strcmp(operand, "0x0") == 0) {
                  snprintf(ret, sizeof(ret), "L%d:\t%s RX_OK;\n", n, op);
                } else if (strcmp(operand, "0xffffffff") == 0) {
                  snprintf(ret, sizeof(ret), "L%d:\t%s pcn_pkt_controller(ctx, md, reason);\n", n, op);
                } else {
