...
Transparenthelloworld::Transparenthelloworld(const std::string name,
  const TransparenthelloworldJsonObject &conf, CubeType type)
...
void Transparenthelloworld::setIngressAction(const TransparenthelloworldIngressActionEnum &value){
...
void Transparenthelloworld::setEgressAction(const TransparenthelloworldEgressActionEnum &value){
  uint8_t action = static_cast<uint8_t>(value);
  auto t = get_array_table<uint8_t>("action_map", 0, ProgramType::EGRESS);
  t.set(0x0, action);
}
