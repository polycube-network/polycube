...
void Fdb::update(const FdbJsonObject &conf) {
  //This method updates all the object/parameter in Fdb object specified in the conf JsonObject.
...
void Fdb::create(Simplebridge &parent, const FdbJsonObject &conf){
  ...
  //Please remember to call here the create static method for all sub-objects of Fdb.
...
void Fdb::setAgingTime(const uint32_t &value){
  ...
  if(agingTime_ != value){
    try {
      ...
      logger()->error("Failed to reload the code with the new aging time {0}", value);
    }
    agingTime_ = value;
  } else {
    logger()->debug("[Fdb] Request to set aging time but the value is the same as before");
  }
}
...
FdbFlushOutputJsonObject Fdb::flush() {
  ...
  try {
    ...
    logger()->error("Error while flushing the filtering database. Reason: {0}", e.what());
