...
FdbEntry::FdbEntry(Fdb &parent, const FdbEntryJsonObject &conf): parent_(parent) {
...
void FdbEntry::update(const FdbEntryJsonObject &conf) {
  //This method updates all the object/parameter in FdbEntry object specified in the conf JsonObject.
...
void FdbEntry::create(Fdb &parent, const std::string &address, const FdbEntryJsonObject &conf){
  parent.logger()->debug("[FdbEntry] Received request to create new filtering database entry");
  parent.logger()->debug("[FdbEntry] mac: {0}", address);

  if(!conf.portIsSet()) {
    parent.logger()->error("[FdbEntry] You should specify the output port for a STATIC entry");
    throw std::runtime_error("You should specify the output port for a STATIC entry");
  }
  ...
  try {
    ...
    parent.logger()->error("[FdbEntry] Error while inserting entry in the table. Reason {0}", e.what());
    throw;
  }
}

std::shared_ptr<FdbEntry> FdbEntry::constructFromMap(Fdb &parent, const std::string &address,
...
std::shared_ptr<FdbEntry> FdbEntry::getEntry(Fdb &parent, const std::string &address){
  ...
  try {
    ...
    parent.logger()->error("[FdbEntry] Unable to read the map key. {0}", e.what());
    throw std::runtime_error("Map entry not found");
  }

  parent.logger()->debug("[FdbEntry] Filtering database entry read successfully");
}
...
std::vector<std::shared_ptr<FdbEntry>> FdbEntry::get(Fdb &parent){
  ...
  try {
    ...
    parent.logger()->error("[FdbEntry] Error while trying to get the Filtering database table: {0}",
                            e.what());
...
void FdbEntry::remove(Fdb &parent){
  parent.logger()->debug("[FdbEntry] Removing all entries from the filtering database table");

  try {
    auto fwdtable = parent.parent_.get_hash_table<uint64_t, fwd_entry>("fwdtable");
    fwdtable.remove_all();
  } catch (std::exception &e) {
    parent.logger()->error("[FdbEntry] Error while removing entries from filtering database table: {0}", e.what());
    throw std::runtime_error("Error while removing all entries from filtering database table");
  }

  parent.logger()->info("[MapEntry] All entries removed from the filtering database table");
}
...
void FdbEntry::setPort(const std::string &value){
  ...
  try {
    auto fwdtable = parent_.parent_.get_hash_table<uint64_t, fwd_entry>("fwdtable");
    fwdtable.set(map_key, map_value);
    parent_.logger()->debug("[FdbEntry] Port updated");
  } catch (std::exception &e) {
    parent_.logger()->error("[FdbEntry] Error while inserting entry in the table. Reason {0}", e.what());
    throw;
  }
}
