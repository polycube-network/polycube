...
void NattingTable::create(Nat &parent, const std::string &internalSrc,
...
std::shared_ptr<NattingTable> NattingTable::getEntry(
    Nat &parent, const std::string &internalSrc, const std::string &internalDst,
    const uint16_t &internalSport, const uint16_t &internalDport,
    const std::string &proto) {
  // This method retrieves the pointer to NattingTable object specified by its
  // keys.
  try {
    ...
    auto entry = std::make_shared<NattingTable>(
        parent, internalSrc, internalDst, internalSport, internalDport,
        parent.proto_from_string_to_int(proto), newIp, newPort,
        originatingRule);
    return entry;
...
void NattingTable::removeEntry(Nat &parent, const std::string &internalSrc,
                               const std::string &internalDst,
                               const uint16_t &internalSport,
                               const uint16_t &internalDport,
                               const std::string &proto) {
...
std::vector<std::shared_ptr<NattingTable>> NattingTable::get(Nat &parent) {
  ...
  try {
    ...
    for (auto &pair : map_entries) {
      ...
      auto entry = std::make_shared<NattingTable>(
          parent, utils::be_uint_to_ip_string(key.src_ip),
...
void NattingTable::remove(Nat &parent) {
  ...
  auto ingress_table =
      parent.get_hash_table<st_k, st_v>("ingress_session_table");
