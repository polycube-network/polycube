...
Ports::Ports(polycube::service::Cube<Ports> &parent,
  ...
  logger()->debug("Creating a new port with type: {0} and name: {1}",
    PortsJsonObject::PortsTypeEnum_to_string(conf.getType()), conf.getName());
...
Ports::~Ports() {
  parent_.logger()->debug("[Ports] Port {0} destroyed. Type {1}",
    getName(), PortsJsonObject::PortsTypeEnum_to_string(getType()));
}
...
void Ports::create(Nat &parent, const std::string &name, const PortsJsonObject &conf){
...
void Ports::setIp(const std::string &value){
  ...
  if (port_type_ == PortsTypeEnum::EXTERNAL) {
    ...
    throw std::runtime_error("[Ports]: You cannot set the IP for INTERNAL interface");
  }
...
void Ports::validateInternalPortParams(Nat &parent, const PortsJsonObject &conf) {
  if(conf.ipIsSet()) {
    parent.logger()->warn("INTERNAL interface doesn't support ip");
    throw std::runtime_error("You don't need to set the ip address on the internal interface");
  }
...
void Ports::validateExternalPortParams(Nat &parent, const PortsJsonObject &conf) {
  if(!conf.ipIsSet()) {
    parent.logger()->warn("EXTERNAL interface needs an external IP");
    throw std::runtime_error("Please set the external IP for the EXTERNAL interface");
  }
