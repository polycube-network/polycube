...
Service::Service(Lbrp &parent, const ServiceJsonObject &conf): parent_(parent),
backend_size_(INITIAL_BACKEND_SIZE) {
  ...
  //FIXME: There is another option here instead of creating every backend at time
  //We could just add the backend to the matrix map and call the updateConsistentHashMap
...
void Service::update(const ServiceJsonObject &conf) {
  //This method updates all the object/parameter in Service object specified in the conf JsonObject.
...
void Service::create(Lbrp &parent, const std::string &vip, const uint16_t &vport,
                     const ServiceProtoEnum &proto, const ServiceJsonObject &conf) {
  parent.logger()->debug("[Service] Received request to create new service entry");
  parent.logger()->debug("[Service] Virtual IP: {0}, virtual port: {1}, protocol: {2}",
                          vip, vport, ServiceJsonObject::ServiceProtoEnum_to_string(proto));
  ...
  if (proto == ServiceProtoEnum::ALL) {
    ...
    Service::create(parent, vip, vport, ServiceProtoEnum::TCP, new_conf);

    //Now is the UDP turn
    new_conf.setProto(ServiceProtoEnum::UDP);
    Service::create(parent, vip, vport, ServiceProtoEnum::UDP, new_conf);

    //Finally, is ICMP turn. For ICMP we use a "special" port since this field is useless
    new_conf.setProto(ServiceProtoEnum::ICMP);
    new_conf.setVport(ICMP_EBPF_PORT);
    Service::create(parent, vip, ICMP_EBPF_PORT, ServiceProtoEnum::ICMP, new_conf);

    //We completed all task, let's directly return since we do not have to execute the code below
    return;
  } else if(proto == ServiceProtoEnum::ICMP && conf.getVport() != ICMP_EBPF_PORT) {
    throw std::runtime_error("ICMP Service requires 0 as virtual port. Since this parameter is useless for ICMP services");
  }

  ServiceKey key = ServiceKey(vip, vport, Service::convertProtoToNumber(proto));
  //The validation of Virtual IP and Port is performed automatically by the framework
  if (parent.service_map_.count(key) != 0) {
    parent.logger()->error("[Service] Key {0}, {1}, {2} already exists in the map",
                           vip, vport, ServiceJsonObject::ServiceProtoEnum_to_string(proto));
    throw std::runtime_error("Key already exists in the service map");
  }

  try {
    std::unordered_map<ServiceKey, Service>::iterator iter;
    bool inserted;
    std::tie(iter, inserted) = parent.service_map_.emplace(std::piecewise_construct,
                                                           std::forward_as_tuple(ServiceKey(conf.getVip(), conf.getVport(), Service::convertProtoToNumber(conf.getProto()))),
                                                           std::forward_as_tuple(parent, conf));

    if (!inserted) {
      throw std::runtime_error("Unable to create the service instance");
    } else {
      parent.logger()->debug("[Service] Service created successfully");
    }
  } catch (std::exception &e) {
    parent.logger()->error("[Service] Error while creating the service");
...
std::shared_ptr<Service> Service::getEntry(Lbrp &parent, const std::string &vip,
  ...
  parent.logger()->debug("[Service] Received request to read a service entry");
  parent.logger()->debug("[Service] Virtual IP: {0}, virtual port: {1}, protocol: {2}",
                         vip, vport, ServiceJsonObject::ServiceProtoEnum_to_string(proto));

  uint16_t vp=vport;
  if(convertProtoToNumber(proto)==1)
    vp=vport;

  ServiceKey key = ServiceKey(vip, vp, Service::convertProtoToNumber(proto));

  if(parent.service_map_.count(key) == 0) {
    parent.logger()->error("[Service] There are no entries associated with that key");
    throw std::runtime_error("There are no entries associated with that key");
  }

  return std::shared_ptr<Service>(&parent.service_map_.at(key), [](Service*){});
}

void Service::removeEntry(Lbrp &parent, const std::string &vip,
                          const uint16_t &vport, const ServiceProtoEnum &proto){
  if (proto == ServiceProtoEnum::ALL) {
    //Let's create 3 different services for TCP, UDP and ICMP
    //Let's start from TCP
     Service::removeEntry(parent, vip, vport, ServiceProtoEnum::TCP);

    //Now is the UDP turn
     Service::removeEntry(parent, vip, vport, ServiceProtoEnum::UDP);

    //Finally, is ICMP turn. For ICMP we use a "special" port since this field is useless
     Service::removeEntry(parent, vip, ICMP_EBPF_PORT, ServiceProtoEnum::ICMP);

    //We completed all task, let's directly return since we do not have to execute the code below
    return;
  } else if (proto == ServiceProtoEnum::ICMP && vport != ICMP_EBPF_PORT) {
    throw std::runtime_error("ICMP Service requires 0 as virtual port. Since this parameter is useless for ICMP services");
  }

  parent.logger()->debug("[Service] Received request to delete a service entry");
  parent.logger()->debug("[Service] Virtual IP: {0}, virtual port: {1}, protocol: {2}",
                          vip, vport,
  ...
  auto service = Service::getEntry(parent, vip, vport, proto);

  service->removeServiceFromKernelMap();
  parent.service_map_.erase(key);
}

std::vector<std::shared_ptr<Service>> Service::get(Lbrp &parent){
  std::vector<std::shared_ptr<Service>> services_vect;

  for(auto &it : parent.service_map_) {
    ServiceKey key = it.first;
    services_vect.push_back(Service::getEntry(parent, std::get<0>(key), std::get<1>(key), Service::convertNumberToProto(std::get<2>(key))));
  }

  return services_vect;
}

void Service::remove(Lbrp &parent){
  for(auto it = parent.service_map_.begin(); it != parent.service_map_.end();) {
    auto tmp = it;
    it++;
    Service::removeEntry(parent, tmp->second.getVip(), tmp->second.getVport(), tmp->second.getProto());
  }
}
...
void Service::updateConsistentHashMap() {
  logger()->debug("[Service] Updating consistent hash map for service {0}, {1}, {2}",
                  getVip(), getVport(),
...
/*
 * The following function is used to load and update services map in the datapath.
 * The map has :
 * 	-key = struct vip: {ip, port, proto, index}
 * 	-value = struct backend: {ip, port}
 */

void Service::updateKernelServiceMap(const std::vector<std::string> consistent_array) {
  auto services_table = parent_.get_hash_table<vip, backend>(EBPF_SERVICE_MAP);
  auto backend_to_service = parent_.get_hash_table<backend, vip>(EBPF_BACKEND_TO_SERVICE_MAP);
  uint16_t index = 0;
  ...
  //This is a particular value that is used to indicate the size of the backend pool.
  ...
  for (const auto &backend_ip : consistent_array) {
    ...
    auto bck = ServiceBackend::getEntry(*this, backend_ip);
...
std::vector<std::string> Service::getConsistentArray() {
  ...
  /* We have to  scroll a matrix , starting from first column , first row [0,0]. For each cell , i retrieve the number stored in that cell and lookup in the final_backend_list if it exists or
   * not , if not , it is saved in this array(final_backend_list) and we go on in the matrix by checking the next cell [0,1] row0 column 1 ;
   * if Yes, we ho to the next row -> [1,0] , retrieve the number in this cell , lookup in the final_backend_list and so on ;
   ...
   * 1) -2  -> go next colum (continue command to go next for loop and x++ keep allign matrix column and vector of backend's name)
   * 2) X>0 -> check if u can store it in the final_backend_list
   * final_backend_list has 2 types of numbers:
   * 1) -1  -> u can store the number , in this case i check if the entry can be assigned to this backend by checking the number in weight_backend_list[backend_list.at(x)] ; if it is greater
   *	than 0, it is stored and the number is decreased weight_backend_list[backend_list.at(x)]--;
...
/*
 * This function return a map<backend_ip,#entries_assigned> after checking backend's weights.
    We need to calculate the common coefficient between all backends.
    Then we multiply this coefficient for backend's weight in order to get the number of entries assigned to this backend.
 */
std::map<std::string,int> Service::get_weight_backend() {
  ...
  logger()->debug("vec size {0} sum_weight {1} coeff {2} ",vect_size,sum_weight,coeff);
  ...
  while (res > 0) {
    for (auto& backend : service_backends_) {
	    ...
	    logger()->debug("backend {0} weight {1} ",backend.first,weight_back_pool[backend.first]);
