...
static int handle_rx(struct CTXTYPE *ctx, struct pkt_metadata *md) {
  ...
  INPUT:;
    ...
    #endif
      pcn_log(ctx, LOG_DEBUG, "No INPUT chain instantiated. Apply default action for INPUT chain. ");
  ...
  FORWARD:;
    ...
    #endif
      pcn_log(ctx, LOG_DEBUG, "No FORWARD chain instantiated. Apply default action for FORWARD chain. ");
    ...
    if (ip_matched){
      pcn_log(ctx, LOG_DEBUG, "EGRESS Chain Selector. Src ip matched. -->OUTPUT chain. ");
      goto OUTPUT;
    } else{
      pcn_log(ctx, LOG_DEBUG, "EGRESS Chain Selector. Src ip NOT matched. -->PASS. ");
      goto PASS;
    }
  ...
  OUTPUT:;
    ...
    #endif
      pcn_log(ctx, LOG_DEBUG, "No OUTPUT chain instantiated. Apply default action for OUTPUT chain. ");
  ...
  PASS:;
    pcn_log(ctx, LOG_DEBUG, "No hit for OUTPUT chain. Let the packet PASS_NO_LABELING. ");
    return RX_OK;
