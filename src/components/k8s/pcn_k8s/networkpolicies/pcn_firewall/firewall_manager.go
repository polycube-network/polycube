...
func StartFirewallManager(basePath, nodeName string, podController pcn_controllers.PodController) Manager {
	...
	//	Subscribe to events
	podController.Subscribe(pcn_types.Update, fwManager.manageFirewallForPod)
...
func (f *FirewallManager) manageFirewallForPod(pod *core_v1.Pod) {
	...
	//	Already there?
	if _, ok := f.firewalls[ID]; ok {
		l.Debugln("Already have the firewaall for pod", pod.Name, "in my list.")
		return
	}

	//	Ok add this.
	l.Debugln("Going to add firewall for pod", pod.Name, "in my list.")
	fw := newFirewall(*pod, f.fwAPI)
	if fw == nil {
		l.Debugln("Could not get the firewall for pod", pod.Name)
		return
	}
