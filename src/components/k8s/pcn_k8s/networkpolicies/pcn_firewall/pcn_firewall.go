...
func (d *FirewallManager) EnforcePolicy(policyName, policyType string, policyTime meta_v1.Time, ingress, egress []k8sfirewall.ChainRule, actions []pcn_types.FirewallAction) {
	...
	if _, exists := d.policyTypes[policyName]; exists {
		l.Errorf("Policy %s is already enforced. Will stop here.", policyName)
		return
	}
...
func (d *FirewallManager) definePolicyActions(policyName string, actions []pcn_types.FirewallAction) {
	l := log.New().WithFields(log.Fields{"by": "FirewallManager-" + d.name, "method": "definePolicyActions(" + policyName + ", ...)"})
	l.Debugf("actions: %+v", actions)
	...
	for _, action := range actions {
		...
		if shouldSubscribe {
			...
			unsub, err := d.podController.Subscribe(pcn_types.Update, podQuery, nsQuery, nil, pcn_types.PodAnyPhase, func(pod *core_v1.Pod, prev *core_v1.Pod) {
			...
			if err == nil {
				...
				l.Errorf("Could not subscribe to changes! %s", err)
			}
		}
	}
}
...
func (d *FirewallManager) reactToPod(event pcn_types.EventType, pod *core_v1.Pod, prev *core_v1.Pod, actionKey string, podQuery *pcn_types.ObjectQuery) {
	...
	l := log.New().WithFields(log.Fields{"by": "FirewallManager-" + d.name, "method": "reactToPod(" + pod.Status.PodIP + ", " + actionKey + "...)"})
...
func (d *FirewallManager) CeasePolicy(policyName string) {
	...
	l := log.New().WithFields(log.Fields{"by": "FirewallManager-" + d.name, "method": "CeasePolicy(" + policyName + ")"})

	if _, exists := d.policyTypes[policyName]; !exists {
		l.Errorf("Policy %s is not currently enforced. Will stop here.", policyName)
		return
	}
...
func (d *FirewallManager) deleteRules(fw, direction string, rules []k8sfirewall.ChainRule) error {
	l := log.New().WithFields(log.Fields{"by": "FirewallManager-" + d.name, "method": "deleteRules()"})
	...
	for _, rule := range rules {
		...
		if err != nil {
			l.Errorf("Error while trying to delete this rule: %+v, in %s for firewall %s. Error %s, response: %+v", rule, direction, fw, err.Error(), response)
		}
	}
