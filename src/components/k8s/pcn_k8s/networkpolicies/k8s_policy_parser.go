		...
		for _, i := range ingress {
			...
			for _, f := range i.From {
				...
				if ok {
					...
					action.Templates = d.getConnectionTemplate(pcn_types.Incoming, "", "", pcn_types.ActionForward, ports)
		...
		for _, e := range egress {
			...
			for _, t := range e.To {
				...
				if ok {
					...
					action.Templates = d.getConnectionTemplate(pcn_types.Outgoing, "", "", pcn_types.ActionForward, ports)
					action.Key = d.buildActionKey(action.PodLabels, action.NamespaceLabels, action.NamespaceName)
					fwActions = append(fwActions, action)
				}
			}
